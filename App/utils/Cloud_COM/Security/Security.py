...
keypath = os.path.join(Folder_Dir,'keys')
print(keypath)

def Sign_Encrypt(Data):
    with open(os.path.join(keypath,"private_sign.pem"), "rb") as key_file:
        sign_key = serialization.load_pem_private_key(
            key_file.read(),
            password=None
        )

    with open(os.path.join(keypath,"public_data.pem"),"rb") as data_key:
...
def Verify_Decrypt(Enc_data):
    with open(os.path.join(keypath,"public_sign.pem"), "rb") as key_file:
        verify_key = serialization.load_pem_public_key(
            key_file.read()
        )

    with open(os.path.join(keypath,"private_data.pem"),"rb") as data_key:
