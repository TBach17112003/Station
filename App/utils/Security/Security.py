...
def Sign_Encrypt(Data):
    with open("./utils/Security/keys/private_sign.pem", "rb") as key_file:
        sign_key = serialization.load_pem_private_key(
            key_file.read(),
            password=None
        )

    with open("./utils/Security/keys/public_data.pem","rb") as data_key:
...
def Verify_Decrypt(Enc_data):
    with open("./utils/Security/keys/public_sign.pem", "rb") as key_file:
        verify_key = serialization.load_pem_public_key(
            key_file.read()
        )

    with open("./utils/Security/keys/private_data.pem","rb") as data_key:
